...
public class SessionTrack {
    ...
    // -- CONSTRUTORES
    /**
     * Cria uma instância dessa classe de acordo com as configurações dos
     parâmetros.
     * @param parameters Um objeto que implemente a interface 'SessionParameter'
    estipulando os parâmetros que o usuário deseja para a sessão.
     */
    ...
    public void cancelSession() {
        if (sessionState == SessionState.Running || sessionState ==
                SessionState.Paused) {
    ...
    // -- MÉTODOS PRIVADOS
    /**
     * Avalia os critérios da atividade selecionada com o estado atual da sessão
     para determinar se ela foi concluída.
     */
    private void checkCompletion() {
        // Verifica se esta medindo as repetições e se o limite foi atingido
        if (parameters.getRepetitions() > 0 && currentRepetitions >=
                parameters.getRepetitions()) {
     ...
     * @param isCancelled Flag indicando se a sessão foi conluida naturalmente ou
    se foi cancelada.
     */
    private void completeSession(boolean isCancelled) {
        // Verifica se o estado da sessão não é cancelado ou completo
        if (sessionState != SessionState.Cancelled && sessionState !=
                SessionState.Completed) {
            chronometer.stop();
            sessionState = isCancelled ? SessionState.Cancelled :
                    SessionState.Completed;
        }
    }
    //
    // -- TIPOS INTERNOS
    /**
     * Definição de interface para um callback que é chamado quando o tempo da
     sessão atualiza seus valores.
     */
